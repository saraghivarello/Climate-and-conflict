...
data_norm <- read.csv("/home/sara/Documenti/GitHub/Climate-and-conflict/csv/df_norm1.csv")
...
form_ban_log1 <- Disp_log1 ~ x + TA_lag2 + PA_lag2 + DL_lag2 + inv_distance_2 + conflicts  + gdp_mean + population_density
...
#log ban 
lmh_b_log1 <- lm(form_ban_log1, data = data_ban_log1)
f_ind_b_log1 <- plm(form_ban_log1, data = data_ban_log1, model = "within", index = c("admin1","month"), effect = "individual")
f_time_b_log1 <- plm(form_ban_log1, data = data_ban_log1, model = "within", index = c("admin1","month"), effect = "time")
f_two_b_log1 <- plm(form_ban_log1, data = data_ban_log1, model = "within", index = c("admin1","month"), effect = "twoways")
